...
#[derive(Clone)]
pub struct UserGrpcManager {
    pub endpoint: Endpoint,
    pub token: String,
}

impl ManageConnection for UserGrpcManager {
    ...
    fn connect(
        ...
        Output = Result<<Self as ManageConnection>::Connection, <Self as ManageConnection>::Error>,
    > + Send {
    ...
    ) -> impl Future<Output = Result<(), <Self as ManageConnection>::Error>> + Send {
        Box::pin(async { Ok(()) })
    }
